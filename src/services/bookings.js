...
export async function saveBookingToFirebase(booking) {
  ...
  const document = await addDoc(bookingsCollection, bookingForFirebase)
  return document.id
}

export function subscribeToBookings(onBookings, onError) {
  ...
  return onSnapshot(
    bookingsQuery,
    (snapshot) => {
      onBookings(snapshot.docs.map((document) => ({ id: document.id, ...document.data() })))
    },
    onError,
  )
}
